...
public class WordGrid {
	...
	public List<List<GridLocation>> generateDomain(String word) {
		...
		for (int row = 0; row < rows; row++) {
            fillDiagonals(domain, length, row);
        }
		return domain;
	}

    private void fillDiagonals(List<List<GridLocation>> domain, int length, int row) {
        for (int column = 0; column < columns; column++) {
            fillRightOnColumns(domain, length, row, column);
            fillRightOnRows(domain, length, row, column);
        }
    }

    private void fillRightOnRows(List<List<GridLocation>> domain, int length, int row, int column) {
        if (row + length <= rows) {
            // top to bottom
    ...
    private void fillRightOnColumns(List<List<GridLocation>> domain, int length, int row, int column) {
